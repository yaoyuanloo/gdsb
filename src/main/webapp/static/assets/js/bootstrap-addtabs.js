...
$.fn.addtabs = function (options) {
    ...
    obj.on('click', '.close_tab', function () {
        var id = $(this).prev("a").attr("aria-controls");
        Addtabs.close(id);
    });


    obj.on('mouseover', '.close_tab', function () {
        //$(this).removeClass('glyphicon-remove').addClass('glyphicon-remove-circle');
    })

    obj.on('mouseout', '.close_tab', function () {
...
window.Addtabs = {
    ...
    add: function (opts) {
		...
		//仅作用当前层级的tab，避免由于tab框架嵌套而关闭父级已激活的标签
		obj.find('li[role = "presentation"].active').removeClass('active'); 
		obj.find('div[role = "tabpanel"].active').removeClass('active');
        //如果TAB不存在，创建一个新的TAB
        if (!$("#" + id)[0]) {
            //创建新TAB的title
