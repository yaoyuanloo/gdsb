...
<body>
	...
	<form:form id="searchForm" modelAttribute="dict" action="${ctx}/sys/dict/" method="post" class="breadcrumb form-search">
		...
		<label>类型：</label><form:select id="type" path="type" class="input-medium" cssStyle="width: 300px;"><form:option value="" label=""/><c:forEach items="${typeList}" var="d"><form:option value="${d.type}" label="${d.description} : ${d.type}"/></c:forEach> <%--<form:options items="${typeList}" itemLabel="description" itemValue="type" htmlEscape="false"/>--%></form:select>
