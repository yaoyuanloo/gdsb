...
<head>
    ...
    <script type="text/javascript">
       ...
       //结束时间
    		...
    		var show=null;
    		var ss=null;
    		tb+='<thead><tr><th>选择</th><th>身份证号</th><th>开始年月</th><th>结束年月</th></tr>';
    		for(var i=0;i<m;i++){
    			...
    			tb+='<tr><td><i class="icon-check-empty"></i></td><td>'+$("#IDNo").val()+'</td><td>'+show+'</td><td>'+show+'</td></tr></thead>'
    		}
        ...
        function thumbnail(imgPath, ywlsh) {
            loading("正在加载，请稍等...");
            top.$.jBox.open("iframe:${ctx}/sisl/busreview/imgshow?imgPath=" + imgPath + "&ywlsh=" + ywlsh, "扫描件 详细", 880, 660, {
                loaded: function () {
                    closeTip();
        ...
        function yewubanjie() {
            ...
            function jbox() {
                top.$.jBox("<div class='form-search' style='padding:20px;text-align:center;'>是否办结？</div>", {
                    title: "提示",
                    submit: function (v, h, f) {
                        if (v == "ok") {
                            var data = {
                            ...
                            $.post("${ctx}/sisl/busreview/qiyebanjie", data, function (data) {
                                if (data.status == 1) {
                                    confirmx(data.msg + "是否返回上一页？", "${ctx}/sisl/busreview/list?pageNo=${pageNo}");
                                } else {
        ...
        function forwording() {
            var ywlsh = $("#ywlsh").val();
            top.$.jBox.open("iframe:${ctx}/sisl/busreview/zhuanjian", "注意事项", 400, 300, {
                ajaxData: {
                    ywlsh: ywlsh
                },
                buttons: {
...
//                    console.log(data);
                    if (v == "qr") {
                        ...
                        $.post("${ctx}/sisl/busreview/receipt", data, function (response) {
                            if (response.success) {
...
//                var reghj = /^[-+]?\d+$/;//户籍正则
            ...
            function jbox() {
                top.$.jBox("<div class='form-search' style='padding:20px;text-align:center;'>是否提交？</div>", {
                    title: "提示",
                    submit: function (v, h, f) {
                        if (v == "ok") {
                            var data = {
...
//                            console.log(data);
                            $.post("${ctx}/sisl/busreview/tijiao", data, function (data) {
                                showTip(data.msg, 'info', 2000, 0);
                            });
...
//                                YWZT:YWZT
                            ...
                            console.log(data);
                            $.post("${ctx}/sisl/busreview/baocun", data, function (data) {
                                showTip(data.msg, 'info', 2000, 0);
                            });
                        }
                    },
...
<div id="myTabContent" class="tab-content">
    <div class="tab-pane active" id="examine">
        ...
        <div>
            <div id="centerDiv" class="breadcrumb form-search" style="margin: 0; text-align: center;">
                <%--<div class="alert" style="font-size: 14px; margin: 0; text-align: center;">
                    <button type="button" class="close" data-dismiss="alert">&lt;%&ndash;&times;&ndash;%&gt;</button>
                    <strong>温馨提示：</strong>此业务【企业首次参保登记】属于企业快捷业务，可直接办结。
                </div>--%>
                ...
                <center>
                    ...
                    <div id="controlForm" class="breadcrumb form-search" style=" margin: 0; border: 0;">
                        ...
                        <%-- <li style=" margin-left: 50px;"><h5 style="line-height: 30px;">
                                 安置,流动,改制时间：</h5>
                            </li>
                            <li style=" margin-left: 10px; line-height: 30px;">
                  ...
                  &nbsp;      
                   ...
                   <li style=""><h5 style="line-height: 30px;">
       ...
       <table id="tabledatas" class="table table-striped table-bordered table-condensed">
                ...
                <ul class="thumbnails" style="margin-top: 5px;">
                    ...
                    <c:forEach items="${imgs}" var="img" varStatus="status">
                        <li class="span3 thumbnail" style="margin: 5px; width: 220px;">
                            <a href="javascript:thumbnail('${img.id}', '${busRecviewBean.YWLSH}');">
