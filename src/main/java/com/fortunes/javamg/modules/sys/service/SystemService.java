...
import com.fortunes.javamg.common.security.Digests;
import com.fortunes.javamg.common.security.shiro.session.SessionDAO;
import com.fortunes.javamg.common.service.BaseService;
import com.fortunes.javamg.common.service.ServiceException;
...
import com.fortunes.javamg.modules.sys.dao.MenuDao;
import com.fortunes.javamg.modules.sys.dao.RoleDao;
...
import com.fortunes.javamg.modules.sys.security.SystemAuthorizingRealm;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	@Autowired
	private GtUserQxDao gtuserqxDao;
	@Autowired
	private GtYwinfoDao gtywinfoDao;
	@Autowired
	private GtUserSlroleDao gtUserSlroleDao;
	@Autowired
	...
	*获取当前地方市的角色列表过滤
	*/
	public List<Role> findAllRoleDi(){
			return UserUtils.getRoleListDigl();
		}
	...
	*/
	...
	public List<GtYwinfo> findgtYwinfoBylsh(String ywlsh){
		return gtywinfoDao.findywinfo(ywlsh);
	}
	...
	*/
	...
	public Page<GtYwinfo> findgtywinfList(Page<GtYwinfo> page, GtYwinfo gtywinfo){
		
		gtywinfo.setPage(page);
		
		page.setList(gtywinfoDao.findywinfoList(gtywinfo));
		
		return page;
		
	}
	public List<User> findUserListByUserQxId(Map m){
		return userDao.findUserListByUserQxId(m);
	}
	 ...
	 * @return
	 */
	public Page<User> findUserDi(Page<User> page, User user) {
		// 设置分页参数
		user.setPage(page);
		// 执行分页查询
...
//			userTemp.setCredNum(AESUtil.decrypt(userTemp.getCredNum(), AESUtil.AES_KEY));
//		}
//		
		return page;
	}
	 ...
	 * @return
	 */
	public List<GtRole> findGtRole(GtRole role){
		return gtroleDao.findList(role);
	}
	...
	public GtUserQx getUserIdByQxId(String userId){
		return gtuserqxDao.getUserIdByQxId(userId);
	}
	 ...
	 * @return
     */
	public GtUserSlrole getGtUserSlrole(String userId){
		return gtUserSlroleDao.getUserIdByRoleId(userId);
	}
	/**
	 * 无分页查询人员列表
	 * @param user
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static String entryptPassword(String plainPassword) {
		byte[] salt = Digests.generateSalt(SALT_SIZE);//8
		byte[] hashPassword = Digests.sha1(plainPassword.getBytes(), salt, HASH_INTERATIONS);//20
	...
	public static void main(String[] args) {
		...
		String password = entryptPassword(plainPassword);
		System.out.println(password);
		System.out.println(validatePassword(plainPassword, password));
	}
	/**
	 * 柜台受理角色-权限与用户关联
	 * @param userid
	...
	public  void  deleteUserRole(String userid){
		userDao.deleteywUserrole(userid);
	}
	@Transactional(readOnly = false)
	public void deleteUserQx(String userid){
		userDao.deleteywUserqx(userid);
	}
	/**
	 * 获得活动会话
	 * @return
